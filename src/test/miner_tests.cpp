...
struct MinerTestingSetup : public TestingSetup {
    MinerTestingSetup()
        : TestingSetup{ChainType::REGTEST, TestOpts{std::vector<const char*>{"-testactivationheight=csv@1000000"}, /*coins_db_in_memory=*/true, /*block_tree_db_in_memory=*/true, /*setup_net=*/true, /*setup_validation_interface=*/true, /*min_validation_cache=*/false}}
    {}
...
void MinerTestingSetup::TestPackageSelection(const CScript& scriptPubKey, const std::vector<CTransactionRef>& txFirst)
{
    ...
    // Avoid 20-minute min-difficulty fallback on regtest by aligning mock time to tip MTP
...
void MinerTestingSetup::TestBasicMining(const CScript& scriptPubKey, const std::vector<CTransactionRef>& txFirst, int baseheight)
{
    ...
    // Use BlockAssembler directly here to honor options.test_block_validity=false.
    // Going through interfaces::Mining would downcast Options to BlockCreateOptions
    // and lose the test_block_validity flag, causing TestBlockValidity to run.
    auto block_template_direct = node::BlockAssembler{m_node.chainman->ActiveChainstate(), m_node.mempool.get(), options}.CreateNewBlock();
    BOOST_REQUIRE(block_template_direct);

    // None of the of the absolute height/time locked tx should have made
    // it into the template because we still check IsFinalTx in CreateNewBlock,
    // but relative locked txs will if inconsistently added to mempool.
    // For now these will still generate a valid template until BIP68 soft fork.
    // If CSV (BIP68) is active for the next block, the relative-locked txs
    // must be excluded.
    CBlock block{block_template_direct->block};
    const bool csv_active_next = (Assert(m_node.chainman)->ActiveChain().Tip()->nHeight + 1) >= Assert(m_node.chainman)->GetConsensus().CSVHeight;
    if (csv_active_next) {
        // Only coinbase should be present in this specific setup
    ...
    BOOST_REQUIRE(block_template);
    block = block_template->getBlock();
    // After advancing height/time, all relative-locked txs become final
    // and should be mined regardless of activation.
...
BOOST_AUTO_TEST_CASE(CreateNewBlock_validity)
{
    ...
    for (const auto& bi : BLOCKINFO) {
        ...
        CMutableTransaction txCoinbase(*block.vtx[0]);
        {
            ...
            // Initialize proof-of-work blob for TensorCash and match consensus defaults
            ...
            // Ensure Tensor header defaults and valid PoW
            InitializeTensorHeader(block);
            while (!CheckProofOfWork(block.GetShortHash(), block.nAdjBits, Params().GetConsensus())) {
                ++block.nNonce;
            }
            block_template->updateBlockPOW(block.pow, block.cumulative_tick, block.nAdjBits);
        }
        ...
        }
        {
            ...
            // we explicitly check this, and align our local block view if
            // miner-modified fields (e.g. commitments) changed the hash.
            auto maybe_new_tip{Assert(m_node.chainman)->ActiveChain().Tip()};
            if (maybe_new_tip->GetBlockHash() != block.GetHash()) {
                CBlock accepted;
                bool ok = Assert(m_node.chainman)->m_blockman.ReadBlock(accepted, *maybe_new_tip);
                BOOST_REQUIRE(ok);
