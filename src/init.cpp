...
void SetupServerArgs(ArgsManager& argsman, bool can_listen_ipc)
{
    ...
    argsman.AddArg("-miningbuildaheadpeers", "Allow broker mining to build one speculative level on a peer-originated block after local Quick/Smell approval while its Full verdict is pending (default: 0). This avoids known-stale sibling work during Full validation but can waste work if Full later rejects the peer block.", ArgsManager::ALLOW_ANY, OptionsCategory::EXTERNAL_API);
...
bool AppInitParameterInteraction(const ArgsManager& args)
{
    ...
    // TIP-0003: refuse to run a chain that activates v3 without
